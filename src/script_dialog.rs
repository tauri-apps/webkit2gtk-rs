...
use ffi;
use glib::translate::{from_glib, mut_override, IntoGlib, Stash, ToGlibPtr};
use glib_sys;
...
impl ScriptDialog {
    ...
    pub fn get_dialog_type(&self) -> ScriptDialogType {
        unsafe {
            from_glib(ffi::webkit_script_dialog_get_dialog_type(mut_override(
                self.to_glib_none().0,
            )))
        }
    }
    ...
    pub fn get_message(&self) -> &str {
        let c_str =
            unsafe { ffi::webkit_script_dialog_get_message(mut_override(self.to_glib_none().0)) };
    ...
    pub fn prompt_get_default_text(&self) -> &str {
        let c_str = unsafe {
            ffi::webkit_script_dialog_prompt_get_default_text(mut_override(self.to_glib_none().0))
        };
        let c_str = unsafe { CStr::from_ptr(c_str) };
        c_str.to_str().unwrap()
    }
...
fn webkit_script_dialog_copy(
    dialog: *const ffi::WebKitScriptDialog,
) -> *mut ffi::WebKitScriptDialog {
