...
pub trait UserContentManagerExt: IsA<UserContentManager> + sealed::Sealed + 'static {
    ...
    fn register_script_message_handler_with_reply(&self, name: &str, world_name: &str) -> bool {
        unsafe {
            from_glib(ffi::webkit_user_content_manager_register_script_message_handler_with_reply(self.as_ref().to_glib_none().0, name.to_glib_none().0, world_name.to_glib_none().0))
        }
    }
    ...
    fn connect_script_message_received<F: Fn(&Self, &JavascriptResult) + 'static>(&self, detail: Option<&str>, f: F) -> SignalHandlerId {
        unsafe extern "C" fn script_message_received_trampoline<P: IsA<UserContentManager>, F: Fn(&P, &JavascriptResult) + 'static>(this: *mut ffi::WebKitUserContentManager, value: *mut ffi::WebKitJavascriptResult, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(UserContentManager::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(value))
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, signal_name.as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(script_message_received_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
