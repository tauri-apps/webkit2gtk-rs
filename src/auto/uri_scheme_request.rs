...
pub trait URISchemeRequestExt: IsA<URISchemeRequest> + sealed::Sealed + 'static {
    ...
    fn http_body(&self) -> Option<gio::InputStream> {
        unsafe {
            from_glib_full(ffi::webkit_uri_scheme_request_get_http_body(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn http_headers(&self) -> Option<soup::MessageHeaders> {
        unsafe {
            from_glib_none(ffi::webkit_uri_scheme_request_get_http_headers(self.as_ref().to_glib_none().0))
        }
    }
