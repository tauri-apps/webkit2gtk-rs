...
impl UserStyleSheet {
    ...
    pub fn new(source: &str, injected_frames: UserContentInjectedFrames, level: UserStyleLevel, allow_list: &[&str], block_list: &[&str]) -> UserStyleSheet {
        ...
        unsafe {
            from_glib_full(ffi::webkit_user_style_sheet_new(source.to_glib_none().0, injected_frames.to_glib(), level.to_glib(), allow_list.to_glib_none().0, block_list.to_glib_none().0))
        }
    }
    ...
    pub fn for_world(source: &str, injected_frames: UserContentInjectedFrames, level: UserStyleLevel, world_name: &str, allow_list: &[&str], block_list: &[&str]) -> UserStyleSheet {
        assert_initialized_main_thread!();
        unsafe {
            from_glib_full(ffi::webkit_user_style_sheet_new_for_world(source.to_glib_none().0, injected_frames.to_glib(), level.to_glib(), world_name.to_glib_none().0, allow_list.to_glib_none().0, block_list.to_glib_none().0))
        }
    }
}
